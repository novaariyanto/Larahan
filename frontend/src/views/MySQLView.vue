...
import { MySQLService } from '../api/services'
...
async function load() {
  try {
    info.value = (await MySQLService.GetInfo()) as ComponentInfo
  } catch (err) {
    console.warn(err)
  }
}
...
async function call(method: 'Install' | 'Uninstall' | 'Start' | 'Stop' | 'Restart') {
  ...
  busy.value = true
  try {
    const result = await MySQLService[method]()
    if (result.ok) {
...
<template>
  <section class="space-y-6">
    <header>
      <h2 class="font-display text-2xl font-semibold text-slate-900">MySQL</h2>
      <p class="mt-1 text-sm text-slate-500">Install, kontrol proses, dan lihat status MySQL.</p>
    </header>
